...
try:
    ...
    wmi = None
    print("Warning: wmi module not available. Windows-specific functionality may be limited.")
    
try:
    from bidi.algorithm import get_display  # Library to handle bidirectional display of strings
except ImportError:
    # Fallback if bidi is not available
                ...
                for addr in addrs:
                    if (addr.family == socket.AF_INET and 
                        ...
                        try:
                            ...
                            self.logger.debug(f"Error processing interface {iface}: {e}")
        ...
        try:
            if sys.platform.startswith('win') and wmi:
                ...
                try:
                    ...
                    self.logger.debug(f"Error getting subnet mask via ip command: {e}")
    ...
    def choose_interface(self, interfaces):
        ...
        while True:
            try:
                ...
                self.logger.error(f"Error selecting interface: {e}")
                return None, None, None
